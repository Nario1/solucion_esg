...
class TestEcuacionSegundoGrado(unittest.TestCase):
    ...
    def test_solucionESG_parametrosNumericos_raicesComplejasConjugadas_subTest(self):
        ...
        ecuacionSegundoGrado = EcuacionSegundoGrado()
        items = (
            {"Case": "Caso 01", "a": 1, "b": 1, "c": 1, "RaizEsperada1": complex(-0.50, 0.87), "RaizEsperada2": complex(-0.50, -0.87)},
            {"Case": "Caso 02", "a": 1, "b": 2, "c": 3, "RaizEsperada1": complex(-1.00, 1.41), "RaizEsperada2": complex(-1.00, -1.41)},
        )
        # Do
        for item in items:
            with self.subTest(item["Case"]):
